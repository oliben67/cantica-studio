...
export class SongbookItem extends vscode.TreeItem {
  ...
  constructor(
    ...
    if (isActive) {
      ...
      this.iconPath = fileIcon ?? new vscode.ThemeIcon('file-code');
...
export class SongbooksProvider
  ...
  private _fileIcon: vscode.Uri | undefined;

  setFileIcon(uri: vscode.Uri): void {
    this._fileIcon = uri;
  }
  ...
  private async _readDir(dir: vscode.Uri): Promise<SongbookNode[]> {
    try {
      ...
      for (const [name, uri] of files) {
        nodes.push(new SongbookItem(_displayLabel(name), uri, uri.fsPath === this._activeUri, undefined, this._fileIcon));
      }
