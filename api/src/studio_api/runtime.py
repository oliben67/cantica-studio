...
class ActorRuntime:
    ...
    def _start_ai_actor(self, defn: ActorDef, connector: CanticaConnector) -> str | None:
        ...
        ai_provider = _make_provider(defn.provider, defn.model)
        actor_cls = type(
            defn.name,
            (StudioActor,),
            {
    ...
    def start_health_monitor(self, servers: list[CanticaServerConfig]) -> None:
        """Register a 15-minute health check against all cantica servers.

        On each tick: pings servers up to 3 times (2-minute gap between retries).
        If all three attempts fail, all running actors are stopped.
        """
        if not servers:
            return
                ...
                for server in servers:
                    try:
                        with httpx.Client(timeout=10) as client:
                            r = client.get(f"{server.url}/health")
                            if r.is_success:
                                return True
                    except Exception:
                        pass
                _log.warning("Cantica health check failed (attempt %d/3)", attempt + 1)
            return False

        def _run() -> None:
            if not _check():
                _log.error("All 3 health check attempts failed — stopping all actors")
                self.stop_all()

        self._scheduler.add_job(
            _run,
            CronTrigger.from_crontab("*/15 * * * *"),
            id="cantica-health-monitor",
            replace_existing=True,
        )
        threading.Thread(target=_run, daemon=True, name="cantica-health-startup").start()
        _log.info("Health monitor registered (every 15 min, %d server(s))", len(servers))
    ...
    def pop_resolved_model(self, name: str) -> str | None:
        """Return the Copilot-resolved model for a 'auto' actor, or None if not yet resolved."""
        state = self._actors.get(name)
        if state is None:
            return None
        return getattr(state.provider, "resolved_model", None) or None
